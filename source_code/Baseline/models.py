...
class DownSampleConv(nn.Module):
    def __init__(
        ...
        self.conv = nn.Conv2d(
            in_channels, out_channels, kernel_size=kernel_size, stride=strides, padding=padding
        )
...
class UpSampleConv(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size=4, strides=2, padding=1, dropout_rate=0.0
    ):
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        # ds7 = self.down7(ds6)
        ds8 = self.down8(ds5)
        us1 = self.up1(ds8, ds5)
        # us2 = self.up2(us1, ds6)
        # us3 = self.up3(us2, ds5)
        us4 = self.up4(us1, ds4)
