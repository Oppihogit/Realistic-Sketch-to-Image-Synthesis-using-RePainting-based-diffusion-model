...
def training(
    ...
    def sample_images(batches_done):
        ...
        save_image(
            img_sample, "images/%s/%s.png" % (dataset_name, batches_done), nrow=5, normalize=True
        )
    ...
    for epoch in range(init_epoch, n_epochs):
        ...
        if checkpoint_interval != -1 and epoch % checkpoint_interval == 0:
            torch.save(
                generator.state_dict(), "saved_models/%s/generator_%d.pth" % (dataset_name, epoch)
            )
            torch.save(
...
if __name__ == "__main__":
    path = Path("datasets/all_data_train/combined/clothes")
